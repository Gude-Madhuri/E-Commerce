...
export default function ChangeRoleTable(props) {
  ...
  const roleChangeHandler = async( id ) => {
    const response = await fetch(
      "http://localhost:8080/ecommerce/updateRoleByUserId?id=" + `${id}`+ "&role=admin",
      {
        mode: "cors",
        headers: {
                ...
                .map((row, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell align="center"><Button onClick={ () => roleChangeHandler(row.id)} disabled={row.role==="admin"?true:false} >Add Admin</Button></TableCell>
                    </TableRow>
                  );
                })}
              {emptyRows > 0 && (
